...
class ArRNNStudyTest(unittest.TestCase):
    ...
    def test_command_matches_rmt_protocol_and_enables_expected_aux(self):
        ...
        self.assertIn("model.activation=tanh", command)
        self.assertIn("model.recurrent_init=orthogonal", command)
        self.assertIn("model.recurrent_identity_scale=1.0", command)
