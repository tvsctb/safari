...
block_seed="${BLOCK_INITIALIZATION_SEED:?set BLOCK_INITIALIZATION_SEED}"
suffix="${RUN_SUFFIX:?set RUN_SUFFIX}"
group="${WANDB_GROUP:?set WANDB_GROUP}"
max_epochs="${MAX_EPOCHS:-180}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-block-$suffix}"
mkdir -p "$output_root"
...
common=(
  ...
  model.block_initialization=orthogonal_residual
  model.block_initialization_seed="$block_seed"
  model.embedding_initialization=deterministic_orthogonal_rows
  model.embedding_initialization_std=0.04
...
for seed in 0 1 2 3 4; do
  name="rmt-d32-dctemb-fixedblock-${suffix}-modelseed${seed}-v1"
