...
group="${WANDB_GROUP:-rmt-d32-init-interactions-20260720-v1}"
max_epochs="${MAX_EPOCHS:-180}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-init-interactions}"
mkdir -p "$output_root"
...
memory='initial_memory,forward_queries,inverse_queries'
...
launch() {
  ...
  local name="rmt-d32-${label}-init-interactions-v1"
...
launch memory-position "$memory,$position"
launch blocks-memory "$blocks,$memory"
launch blocks-position "$blocks,$position"
launch blocks-memory-position "$blocks,$memory,$position"
launch embedding-blocks "$embedding,$blocks"
launch embedding-memory-position "$embedding,$memory,$position"
launch all-random-groups "$embedding,$blocks,$memory,$position"
