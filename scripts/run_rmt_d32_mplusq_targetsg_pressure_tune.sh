...
# Fixed target-stop-gradient x memory-pressure screen for the original-RMT-like
# memory-plus-query write input. SG pairs are adjacent so matched cells share a
# GPU shard. This is a fresh architecture-specific screen, not a continuation
# of the legacy query-only pressure tuning.
set -euo pipefail
...
project="${WANDB_PROJECT:-rmt-write-memory}"
group="${WANDB_GROUP:-rmt-d32-mplusq-targetsg-pressure-tune-20260817-v1}"
suffix="${RUN_SUFFIX:-mplusq-targetsg-pressure400-v1}"
max_epochs="${MAX_EPOCHS:-400}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 5))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-mplusq-targetsg-pressure-tune}/shard-${shard}"
mkdir -p "$output_root"
...
common=(
  ...
  task.aux_metric_profile=full
  wandb.mode=online
  wandb.project="$project"
  wandb.group="$group"
)

# Target SG and rho affect only losses. Verify all ten cells have identical
# fresh inference parameters and that both learned write offsets start at zero.
python - <<'PY'
import torch
...
assert all(torch.count_nonzero(model.forward_queries) == 0 for model in models)
assert all(torch.count_nonzero(model.inverse_queries) == 0 for model in models)
print(f"Verified {len(keys)} inference tensors and zero write offsets across all ten configurations")
PY

seeds=(197 198 199 200 201)
...
for ((job_index = start; job_index < end; job_index++)); do
  ...
  name="rmt-d32-mplusq-targetsg-${sg}-p${pressure}-s${seed}-${suffix}"
...
echo "Memory-plus-query target-SG pressure shard $shard launched ${#pids[@]} processes"
