...
group="${WANDB_GROUP:-rmt-d32-mx4-t125-post-solve-decay-20260721-v1}"
...
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-mx4-t125-post-solve-decay}/shard-${shard}"
mkdir -p "$output_root"
...
for seed in "${seeds[@]}"; do
  for post_weight in 0.0 0.1 0.3; do
    label="${post_weight/./p}"
    name="rmt-d32-mx4-t125-postsolve${label}-s${seed}-v1"
    python -m train "${common[@]}" train.seed="$seed" \
      task.aux_post_solve_weight="$post_weight" \
