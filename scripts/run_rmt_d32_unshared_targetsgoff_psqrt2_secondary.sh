...
group="${WANDB_GROUP:-rmt-d32-unshared-targetsgoff-psqrt2-secondary-20260816-v1}"
suffix="${RUN_SUFFIX:-targetsgoff-psqrt2-secondary400-v1}"
max_epochs="${MAX_EPOCHS:-400}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 5))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-unshared-targetsgoff-psqrt2-secondary}/shard-${shard}"
mkdir -p "$output_root"
...
# The interpolated pressure changes only the auxiliary objective, not inference.
python - <<'PY'
import torch
...
selected = RMTAuxLM(
    **base, rho=26.591480, stop_gradient_memory_target=False,
...
for seed in "${seeds[@]}"; do
  name="rmt-d32-stuinv-wd01-fullaux-targetsgoff-psqrt2-s${seed}-${suffix}"
...
echo "SG-off psqrt2 secondary shard $shard launched ${#pids[@]} processes"
