...
# Stability screen for the exact memory-x4 + terminal-1.25 loss setting.
# The completed tied-PE/fixed-AUX runs are reused as the control, so this
# script launches only the three new stabilization conditions.
set -euo pipefail
...
group="${WANDB_GROUP:-rmt-d32-memoryx4-terminal125-stability-screen-20260721-v1}"
suffix="${RUN_SUFFIX:-stability-screen-v1}"
...
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-memoryx4-terminal125-stability}/shard-${shard}"
mkdir -p "$output_root"
...
launch() {
  ...
  echo "Launching $name (split_pe=$([[ "$share_position" == false ]] && echo true || echo false), direction=$direction, aux=$initial_weight->$final_weight/$schedule)"
  python -m train \
    "${common[@]}" \
    train.seed="$seed" \
...
# Each shard runs one seed from every new condition (three processes/GPU).
seed="$shard"
launch splitpe-fixed "$seed" false false fixed 1.0 1.0 1
launch tied-warmup "$seed" true true linear 0.1 1.0 "$aux_warmup_steps"
launch splitpe-warmup "$seed" false false linear 0.1 1.0 "$aux_warmup_steps"
