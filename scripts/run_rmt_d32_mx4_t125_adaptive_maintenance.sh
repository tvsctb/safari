...
group="${WANDB_GROUP:-rmt-d32-mx4-t125-adaptive-maintenance-20260721-v1}"
max_epochs="${MAX_EPOCHS:-240}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 10))"
decay_steps="$((training_steps / 20))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-mx4-t125-adaptive-maintenance}/shard-${shard}"
mkdir -p "$output_root"
...
common=(
  ...
  task.aux_weight=0.3 task.aux_weight_final=1.0 task.aux_weight_schedule=linear
  task.aux_weight_decay_start_step=0
  task.aux_weight_decay_end_step="$warmup_steps"
  task.aux_activation_lm_loss_threshold=null
  task.aux_solved_ce_threshold=0.05 task.aux_solved_ce_ema_beta=0.99
  task.aux_post_solve_weight=0.5 task.aux_post_solve_hold_steps=0
  task.aux_post_solve_decay_steps="$decay_steps"
...
for seed in "${seeds[@]}"; do
  for cutoff_percent in 35 40 45; do
    cutoff_steps="$((training_steps * cutoff_percent / 100))"
    name="rmt-d32-mx4-t125-adaptmaint-cut${cutoff_percent}p-s${seed}-v1"
    python -m train "${common[@]}" train.seed="$seed" \
      task.aux_post_solve_adaptive_end_step="$cutoff_steps" \
