...
# Secondary paired evaluation of the selected target-SG-off, pressure-2
# full-AUX configuration on seeds with an existing locked no-AUX baseline.
set -euo pipefail
...
group="${WANDB_GROUP:-rmt-d32-unshared-targetsgoff-p2-secondary-20260814-v1}"
suffix="${RUN_SUFFIX:-targetsgoff-p2-secondary400-v1}"
max_epochs="${MAX_EPOCHS:-400}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 5))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-unshared-targetsgoff-p2-secondary}/shard-${shard}"
mkdir -p "$output_root"
...
# Target SG, rho, and AUX component switches affect training objectives only.
# Verify the selected configuration has the same inference initialization as
# the pre-existing no-AUX baseline configuration on the paired seeds.
python - <<'PY'
import torch
...
selected = RMTAuxLM(
    **base, rho=22.360680, stop_gradient_memory_target=False,
...
keys = [key for key in selected_state if key.startswith(prefixes)]
assert keys
assert all(torch.equal(selected_state[key], baseline_state[key]) for key in keys)
...
for seed in "${seeds[@]}"; do
  name="rmt-d32-stuinv-wd01-fullaux-targetsgoff-p200-s${seed}-${suffix}"
  echo "Launching $name"
  python -m train \
    "${common[@]}" \
    train.seed="$seed" \
...
echo "Target-SG-off pressure-2 shard $shard launched ${#pids[@]} processes"
