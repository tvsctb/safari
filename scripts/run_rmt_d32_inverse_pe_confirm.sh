...
group="${WANDB_GROUP:-rmt-d32-inverse-pe-confirm-20260720-v1}"
max_epochs="${MAX_EPOCHS:-180}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-inverse-pe-confirm}"
mkdir -p "$output_root"
...
launch() {
  ...
  local name="rmt-d32-${label}-seed${seed}-inverse-pe-confirm-v1"
