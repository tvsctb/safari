...
# Full-length paired AdamW weight-decay screen for the selected natural
# shared-token/head, unshared-inverse RMT recipe at the exact scale anchor.
set -euo pipefail
...
group="${WANDB_GROUP:-rmt-d32-shared-token-unshared-inverse-wd-20260811-v1}"
suffix="${RUN_SUFFIX:-stuinvwd400-v1}"
max_epochs="${MAX_EPOCHS:-400}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 5))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-shared-token-unshared-inverse-wd}/shard-${shard}"
mkdir -p "$output_root"
...
if (( ${#seeds[@]} * ${#conditions[@]} > 8 )); then
  echo "Refusing to exceed 8 processes on one GPU" >&2
  exit 2
fi
...
for seed in "${seeds[@]}"; do
  for condition in "${conditions[@]}"; do
    IFS='|' read -r label weight_decay <<<"$condition"
...
echo "Weight-decay shard $shard launched ${#pids[@]} processes"
status=0
