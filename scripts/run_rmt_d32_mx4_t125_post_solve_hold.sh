...
group="${WANDB_GROUP:-rmt-d32-mx4-t125-post-solve-hold-20260721-v1}"
...
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-mx4-t125-post-solve-hold}/shard-${shard}"
mkdir -p "$output_root"
...
common=(
  ...
  task.aux_post_solve_weight=0.0 task.aux_post_solve_decay_steps="$decay_steps"
...
for seed in "${seeds[@]}"; do
  for hold_percent in 10 20 30; do
    hold_steps="$((training_steps * hold_percent / 100))"
    name="rmt-d32-mx4-t125-postsolve-hold${hold_percent}p-s${seed}-v1"
    python -m train "${common[@]}" train.seed="$seed" \
      task.aux_post_solve_hold_steps="$hold_steps" \
