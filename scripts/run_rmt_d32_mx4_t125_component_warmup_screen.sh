...
group="${WANDB_GROUP:-rmt-d32-mx4-t125-component-warmup-20260721-v1}"
max_epochs="${MAX_EPOCHS:-240}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 10))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-mx4-t125-component-warmup}/shard-${shard}"
mkdir -p "$output_root"
...
common=(
  ...
  task.aux_weight_schedule=linear task.aux_weight_decay_start_step=0
...
launch() {
  ...
  local condition="$2"
  shift 2
  local name="rmt-d32-mx4-t125-${condition}-s${seed}-component-warmup-v1"
...
for seed in "${seeds[@]}"; do
  launch "$seed" token-warm \
    +task.aux_component_weights.chunk_ce=0.3 \
    +task.aux_component_weights.discrete_ce=0.3 \
    +task.aux_component_weights_final.chunk_ce=1.0 \
    +task.aux_component_weights_final.discrete_ce=1.0
  launch "$seed" state-warm \
    +task.aux_component_weights.memory_nll=0.3 \
    +task.aux_component_weights.terminal_nll=0.3 \
    +task.aux_component_weights_final.memory_nll=1.0 \
    +task.aux_component_weights_final.terminal_nll=1.0
  launch "$seed" discrete-warm \
    +task.aux_component_weights.discrete_ce=0.3 \
    +task.aux_component_weights_final.discrete_ce=1.0
done
