...
group="${WANDB_GROUP:-rmt-d32-mx4-t125-observation-sg-20260721-v1}"
...
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-mx4-t125-observation-sg}/shard-${shard}"
mkdir -p "$output_root"
...
common=(
  ...
  model.stop_gradient_memory_observation=true
...
launch() {
  ...
  local condition="$2"
  shift 2
  local name="rmt-d32-mx4-t125-obssg-${condition}-s${seed}-v1"
...
for seed in "${seeds[@]}"; do
  ...
  launch "$seed" floor03 \
    task.aux_weight=0.3 task.aux_weight_final=1.0 \
    task.aux_weight_schedule=linear task.aux_weight_decay_start_step=0 \
    task.aux_weight_decay_end_step="$warmup_steps"
  launch "$seed" delay5 \
    task.aux_weight=0.0 task.aux_weight_final=1.0 \
    task.aux_weight_schedule=linear \
