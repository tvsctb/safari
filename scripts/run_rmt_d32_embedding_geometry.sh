...
mode="${EMBEDDING_INITIALIZATION:?set EMBEDDING_INITIALIZATION}"
...
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-embedding-$suffix}"
mkdir -p "$output_root"
...
common=(
  ...
  model.embedding_initialization="$mode" model.embedding_initialization_std=0.04
...
for seed in 0 1 2 3 4; do
  name="rmt-d32-orthres-embgeom-${suffix}-seed${seed}-v1"
