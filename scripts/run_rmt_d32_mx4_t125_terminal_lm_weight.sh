...
group="${WANDB_GROUP:-rmt-d32-mx4-t125-terminal-lm-weight-20260721-v1}"
max_epochs="${MAX_EPOCHS:-240}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 10))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-mx4-t125-terminal-lm-weight}/shard-${shard}"
mkdir -p "$output_root"
...
for seed in "${seeds[@]}"; do
  for terminal_weight in 4 8 16; do
    name="rmt-d32-mx4-t125-terminal-lm${terminal_weight}-s${seed}-v1"
    python -m train "${common[@]}" train.seed="$seed" \
      task.lm_terminal_token_weight="$terminal_weight" \
