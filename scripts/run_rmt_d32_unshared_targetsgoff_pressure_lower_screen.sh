...
group="${WANDB_GROUP:-rmt-d32-unshared-targetsgoff-pressure-lower-20260815-v1}"
suffix="${RUN_SUFFIX:-targetsgoff-pressure-lower400-v1}"
max_epochs="${MAX_EPOCHS:-400}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 5))"
output_root="${OUTPUT_ROOT:-/output/rmt-d32-unshared-targetsgoff-pressure-lower}/shard-${shard}"
mkdir -p "$output_root"

# Sparse mechanistic diagnostics keep W&B readable while retaining early,
# middle, and final gradient windows for the selected pressure analysis.
...
# Pressure changes only the training objective. The forward-path parameters
# must have identical seeded initialization across all four candidates.
python - <<'PY'
import torch
...
models = []
for rho in (44.721360, 31.622777, 26.591480, 22.360680):
...
print("Verified 30 inference tensors across all four SG-off pressures")
PY

pressure_labels=(0p5 1 sqrt2 2)
rhos=(44.721360 31.622777 26.591480 22.360680)
pids=()
names=()
for seed in "${seeds[@]}"; do
  for index in "${!rhos[@]}"; do
    pressure="${pressure_labels[$index]}"
    rho="${rhos[$index]}"
...
echo "SG-off lower-pressure shard $shard launched ${#pids[@]} processes"
