...
group="${WANDB_GROUP:-rmt-d32-inverse-pe-independent-20260720-v1}"
max_epochs="${MAX_EPOCHS:-180}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-inverse-pe-independent}"
mkdir -p "$output_root"
...
common=(
  ...
  model.inverse_position_initialization=independent
...
for seed in 0 1 2 3 4; do
  name="rmt-d32-splitpe-independent-nodir-seed${seed}-v1"
