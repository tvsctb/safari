...
# Full-length paired static rho/tau coordinate screen for the selected
# shared-token/head, unshared-inverse RMT recipe.
set -euo pipefail
...
group="${WANDB_GROUP:-rmt-d32-shared-token-unshared-inverse-scale-20260811-v1}"
suffix="${RUN_SUFFIX:-stuinvscale400-v1}"
max_epochs="${MAX_EPOCHS:-400}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 5))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-shared-token-unshared-inverse-scale}/seed-${seed}"
mkdir -p "$output_root"
...
# Effective loss pressure is proportional to 1/scale^2. These are exact
# +/-10% pressure moves around rho=31.622777 and tau=11.925695.
conditions=(
  "anchor|31.622777|11.925695"
  "rho-weak10|33.333334|11.925695"
  "rho-strong10|30.151135|11.925695"
  "tau-weak10|31.622777|12.570786"
  "tau-strong10|31.622777|11.370704"
)

if (( ${#conditions[@]} > 8 )); then
  echo "Refusing to run ${#conditions[@]} processes on one GPU" >&2
  exit 2
fi
...
for condition in "${conditions[@]}"; do
  ...
  name="rmt-d32-stuinv-scale-${label}-s${seed}-${suffix}"
  echo "Launching $name"
  python -m train \
    "${common[@]}" \
    train.seed="$seed" \
...
echo "Scale shard $shard launched ${#pids[@]} processes"
status=0
