...
group="${WANDB_GROUP:-rmt-d32-inverse-pe-screen-20260720-v1}"
max_epochs="${MAX_EPOCHS:-180}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-inverse-pe-screen}"
mkdir -p "$output_root"
...
pids=(); names=()
launch() {
  local label="$1" seed="$2" share_position="$3" direction="$4"
  local name="rmt-d32-${label}-seed${seed}-inverse-pe-v1"
...
for seed in 0 2; do
  launch tied-dir "$seed" true true
  launch tied-nodir "$seed" true false
  launch splitpe-nodir "$seed" false false
  launch splitpe-dir "$seed" false true
done
