...
group="${WANDB_GROUP:-rmt-d32-mx4-t125-lm-gate-20260721-v1}"
max_epochs="${MAX_EPOCHS:-240}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 10))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-mx4-t125-lm-gate}/shard-${shard}"
mkdir -p "$output_root"
...
common=(
  ...
  task.aux_activation_lm_loss_ema_beta=0.99
...
for seed in "${seeds[@]}"; do
  for threshold in 2.05 2.10 2.15; do
    label="${threshold/./p}"
    name="rmt-d32-mx4-t125-lmgate${label}-s${seed}-v1"
    python -m train "${common[@]}" train.seed="$seed" \
      task.aux_activation_lm_loss_threshold="$threshold" \
