...
# Natural full-length LR search around the locked static tau-weak AUX setup.
# No adaptive rules, loss schedules, or answer-token weighting are used.
set -euo pipefail
...
group="${WANDB_GROUP:-rmt-d32-tauweak-lr-full-screen-20260722-v1}"
suffix="${RUN_SUFFIX:-lrfull400-v1}"
max_epochs="${MAX_EPOCHS:-400}"
steps_per_epoch=157
training_steps="$((steps_per_epoch * max_epochs))"
warmup_steps="$((training_steps / 10))"
output_root="${OUTPUT_ROOT:-/tmp/rmt-d32-tauweak-lr-full-screen}/shard-${shard}"
mkdir -p "$output_root"
...
jobs=()
for seed in 110 111 112 113 114; do
  jobs+=("0.001|1e-3|$seed")
  jobs+=("0.002|2e-3|$seed")
  jobs+=("0.003|3e-3|$seed")
done
...
for index in "${!jobs[@]}"; do
  ...
  IFS='|' read -r lr lr_label seed <<<"${jobs[$index]}"
  name="rmt-d32-tauweak-static-aux-lr${lr_label}-s${seed}-${suffix}"
  echo "Launching $name"
  python -m train \
    "${common[@]}" \
    train.seed="$seed" \
