    ...
    def forward(self, batch, encoder, model, decoder, _state):
        ...
        if compute_aux:
            ...
            if should_log_gradient_norms:
                ...
                for scale, scale_components in getattr(
                    model, "scale_loss_components", {}
                ).items():
                    weighted_scale, _ = weighted_aux_components(
                        scale_components, self._current_aux_component_weights
                    )
                    scale_metrics = auxiliary_gradient_norm_metrics(
                        model,
                        w["metric_loss"],
                        weighted_scale,
                        self._current_aux_weight,
